...
echo "=========================================="
echo "Lab 4: CDI - Docker + Open Liberty + PostgreSQL"
echo "=========================================="
echo ""
...
echo "----------------------------"
docker build -t banking-cdi-app:lab04 -f Containerfile . -q
...
if docker ps -a | grep -q banking-cdi-lab04; then
    echo "Stopping existing container..."
    docker stop banking-cdi-lab04 > /dev/null 2>&1 || true
    docker rm banking-cdi-lab04 > /dev/null 2>&1 || true
fi
...
while [ $ELAPSED -lt $TIMEOUT ]; do
    ...
    if ! docker ps | grep -q banking-cdi-lab04; then
        echo ""
        echo -e "${RED}❌ Container stopped unexpectedly${NC}"
        echo ""
        echo "Container logs:"
        echo "----------------------------"
        docker logs banking-cdi-lab04 | tail -50
        exit 1
    fi
...
if [ "$LIBERTY_STARTED" = false ]; then
    ...
    docker ps -a | grep banking-cdi-lab04
    echo ""
...
if docker exec banking-db psql -U bankuser -d bankdb -c "\dt" | grep -q "clients"; then
    ...
    echo "  docker logs banking-cdi-lab04 | grep -i flyway"
fi
...
if [ $TESTS_FAILED -gt 0 ]; then
    ...
    echo "  docker logs banking-cdi-lab04"
    echo "  docker logs banking-cdi-lab04 | grep -i error"
    echo "  docker logs banking-cdi-lab04 | grep -i cdi"
fi
...
echo "  docker logs -f banking-cdi-lab04"
echo ""
echo "View database logs:"
echo "  docker logs -f banking-db"
echo ""
echo "Stop application:"
echo "  docker stop banking-cdi-lab04"
echo ""
echo "Stop database:"
echo "  docker-compose down"
echo ""
echo "Remove application container:"
echo "  docker rm banking-cdi-lab04"
echo ""
echo "Remove application image:"
echo "  docker rmi banking-cdi-app:lab04"
echo ""
echo "Restart application:"
echo "  docker restart banking-cdi-lab04"
echo ""
...
echo ""
echo -e "${GREEN}✓ Lab 4 (CDI) is running with Docker + Open Liberty + PostgreSQL!${NC}"
echo ""
