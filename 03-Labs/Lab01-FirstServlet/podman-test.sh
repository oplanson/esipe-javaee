...
################################################################################
# Lab 01 - First Servlet: Podman Test Script
# 
# This script provides automated testing for Lab 01 with:
...
# Database configuration (not needed for Lab 01)
DB_CONTAINER=""
DB_PORT=5432
DB_USER="bankuser"
...
main() {
    ...
    run_test "Welcome servlet accessible" \
        "curl -f -s http://localhost:${APP_PORT}/welcome >/dev/null"
    
    run_test "Welcome servlet contains 'Welcome'" \
        "curl -s http://localhost:${APP_PORT}/welcome | grep -q 'Welcome'"
    
    run_test "Clients servlet accessible" \
        "curl -f -s http://localhost:${APP_PORT}/clients >/dev/null"
    
    run_test "Clients servlet contains 'Client'" \
    ...
    if [ "$TESTS_FAILED" -eq 0 ]; then
        ...
        echo "  👋 Welcome:  http://localhost:${APP_PORT}/welcome"
