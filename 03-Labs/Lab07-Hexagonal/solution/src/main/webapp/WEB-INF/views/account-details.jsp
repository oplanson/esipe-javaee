        ...
        <main>
            ...
            <% if (account != null && account.getClient() != null) { %>
                <div class="breadcrumb">
                    ...
                    <span>Account <%= account.getAccountNumber() %></span>
                </div>
            <% } %>
            ...
            <% if (account != null) { %>
                ...
                <div class="transaction-form">
                    <h3>💰 Deposit Money</h3>
                    <form method="post" action="<%= request.getContextPath() %>/accounts/<%= account.getId() %>/deposit" style="display: flex; gap: 10px; align-items: flex-end;">
                ...
                <div class="transaction-form">
                    ...
                    <form method="post" action="<%= request.getContextPath() %>/accounts/<%= account.getId() %>/withdraw" style="display: flex; gap: 10px; align-items: flex-end;">
                ...
                <% if (transferEnabled != null && transferEnabled) { %>
                    <div class="transaction-form">
                        ...
                        <form method="post" action="<%= request.getContextPath() %>/accounts/transfer" style="display: flex; gap: 10px; align-items: flex-end; flex-wrap: wrap;">
                            <input type="hidden" name="fromAccountId" value="<%= account.getId() %>">
                ...
                <div class="actions">
                    <a href="<%= request.getContextPath() %>/accounts/<%= account.getId() %>" class="button secondary">
                        ✏️ Edit Account
                    </a>
                    <% if (account.getClient() != null) { %>
                        <a href="<%= request.getContextPath() %>/clients/<%= account.getClient().getId() %>" class="button secondary">
                            ← Back to Client
                        </a>
                    <% } %>
                    ...
                    <% if (deletionEnabled != null && deletionEnabled) { %>
                        <form method="post" action="<%= request.getContextPath() %>/accounts/<%= account.getId() %>/close"
                              ...
                              onsubmit="return confirm('Are you sure you want to close this account? This action cannot be undone.');">
