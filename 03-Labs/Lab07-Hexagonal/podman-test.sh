...
WAR_NAME="banking-hexagonal-app.war"
...
main() {
    ...
    run_test "Account REST adapter (v1)" \
        "curl -f -s http://localhost:${APP_PORT}/api/v1/accounts?clientId=1 > /dev/null"
    ...
    run_test "Account REST adapter (v2)" \
        "curl -f -s http://localhost:${APP_PORT}/api/v2/accounts?clientId=1 > /dev/null"
