...
# Lab 5 Testing Script
# Tests the complete Lab 5 solution with REST API and PostgreSQL

set -e

echo "=========================================="
echo "Lab 5: REST API and Dependency Injection - Test"
echo "=========================================="
echo ""
...
# Check WAR file
if [ -f "target/banking-rest-app.war" ]; then
    echo -e "${GREEN}✓ WAR file created: target/banking-rest-app.war${NC}"
    WAR_SIZE=$(du -h target/banking-rest-app.war | cut -f1)
    echo "  Size: $WAR_SIZE"
else
...
if jar -tf target/banking-rest-app.war | grep -q "WEB-INF/classes/com/bank/model/Client.class"; then
    echo -e "${GREEN}✓ Client entity found${NC}"
else
    echo -e "${RED}❌ Client entity missing${NC}"
fi

if jar -tf target/banking-rest-app.war | grep -q "WEB-INF/classes/com/bank/model/Account.class"; then
    echo -e "${GREEN}✓ Account entity found${NC}"
else
    echo -e "${RED}❌ Account entity missing${NC}"
fi

if jar -tf target/banking-rest-app.war | grep -q "WEB-INF/classes/com/bank/service/ClientService.class"; then
    echo -e "${GREEN}✓ ClientService found${NC}"
else
    echo -e "${RED}❌ ClientService missing${NC}"
fi

if jar -tf target/banking-rest-app.war | grep -q "WEB-INF/classes/META-INF/persistence.xml"; then
    echo -e "${GREEN}✓ persistence.xml found${NC}"
else
    echo -e "${RED}❌ persistence.xml missing${NC}"
fi

if jar -tf target/banking-rest-app.war | grep -q "WEB-INF/classes/db/migration/V1__create_clients_table.sql"; then
...
echo "  - Tests REST API features (Qualifiers, Events, Interceptors)"
echo ""
