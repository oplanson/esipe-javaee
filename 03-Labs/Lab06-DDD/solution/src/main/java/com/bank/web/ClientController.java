...
public class ClientController extends HttpServlet {
    ...
    private void createClient(HttpServletRequest req, HttpServletResponse resp)
        ...
        // Create client using factory method - transaction managed by @Transactional in service
        try {
            Email emailVO = Email.of(email.trim());
            Client client = premium
                ? Client.createPremium(name.trim(), emailVO)
                : Client.create(name.trim(), emailVO);
            ...
            resp.sendRedirect(req.getContextPath() + "/client?action=new&error=invalid_email");
        }
    ...
    private void updateClient(HttpServletRequest req, HttpServletResponse resp)
        ...
        try {
            ...
            // Update client using domain methods - transaction managed by @Transactional in service
            try {
                Email emailVO = Email.of(email.trim());
                client.updateName(name.trim());
                client.updateEmail(emailVO);
                
                // Handle premium status change
                if (premium && !client.isPremium()) {
                    client.upgradeToPremium();
                } else if (!premium && client.isPremium()) {
                ...
                logger.warning("Error updating client: " + e.getMessage());
                resp.sendRedirect(req.getContextPath() + "/client?action=edit&id=" + idParam + "&error=" + e.getMessage());
                return;
            }
