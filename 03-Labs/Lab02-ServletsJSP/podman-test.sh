...
echo "----------------------------"
podman build -t banking-web-app:lab02 -f Containerfile .
...
if podman ps -a | grep -q banking-web-lab02; then
    echo "Stopping existing container..."
    podman stop banking-web-lab02 > /dev/null 2>&1 || true
    podman rm banking-web-lab02 > /dev/null 2>&1 || true
fi
...
while [ $ELAPSED -lt $TIMEOUT ]; do
    ...
    if ! podman ps | grep -q banking-web-lab02; then
        echo ""
        echo -e "${RED}❌ Container stopped unexpectedly${NC}"
        echo ""
        echo "Container logs:"
        echo "----------------------------"
        podman logs banking-web-lab02 | tail -50
        exit 1
    fi
...
if [ "$LIBERTY_STARTED" = false ]; then
    ...
    podman ps -a | grep banking-web-lab02
    echo ""
...
if curl -s -I http://localhost:9080/ | head -1 | grep -q "200\|302"; then
    ...
    podman logs banking-web-lab02 | grep -i "port\|endpoint\|listening" | tail -10
    exit 1
fi
...
HTTP_CODE=$(curl -s -o /tmp/response.txt -w "%{http_code}" "http://localhost:9080/client?action=new")
...
HTTP_CODE=$(curl -s -o /tmp/response.txt -w "%{http_code}" http://localhost:9080/health/live)
if [ "$HTTP_CODE" = "200" ] && grep -q "UP" /tmp/response.txt; then
    echo -e "${GREEN}✓ PASS${NC}"
    TESTS_PASSED=$((TESTS_PASSED + 1))
else
...
HTTP_CODE=$(curl -s -o /tmp/response.txt -w "%{http_code}" http://localhost:9080/health/ready)
if [ "$HTTP_CODE" = "200" ] && grep -q "UP" /tmp/response.txt; then
    echo -e "${GREEN}✓ PASS${NC}"
    TESTS_PASSED=$((TESTS_PASSED + 1))
else
...
if [ $TESTS_FAILED -gt 0 ]; then
    ...
    echo "  podman logs banking-web-lab02"
fi
...
echo ""
echo "View logs:"
echo "  podman logs -f banking-web-lab02"
echo ""
echo "Stop container:"
echo "  podman stop banking-web-lab02"
echo ""
echo "Remove container:"
echo "  podman rm banking-web-lab02"
echo ""
echo "Remove image:"
echo "  podman rmi banking-web-app:lab02"
echo ""
echo "Restart container:"
echo "  podman restart banking-web-lab02"
echo ""
echo "Execute shell in container:"
echo "  podman exec -it banking-web-lab02 /bin/bash"
echo ""
