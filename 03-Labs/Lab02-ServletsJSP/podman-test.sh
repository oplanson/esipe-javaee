...
################################################################################
# Lab 02 - Servlets & JSP: Podman Test Script
# 
# This script provides automated testing for Lab 02 with:
...
# Database configuration (not needed for Lab 02)
DB_CONTAINER=""
DB_PORT=5432
DB_USER="bankuser"
...
main() {
    ...
    run_test "Clients page accessible" \
        "curl -f -s http://localhost:${APP_PORT}/clients >/dev/null"
    
    run_test "Clients page contains 'Client'" \
        "curl -s http://localhost:${APP_PORT}/clients | grep -q 'Client'"
    
    run_test "New client form accessible" \
        "curl -f -s 'http://localhost:${APP_PORT}/client?action=new' >/dev/null"
    
    run_test "Client view page accessible" \
        "curl -f -s 'http://localhost:${APP_PORT}/client?action=view&id=1' >/dev/null"
    ...
    run_test "OpenAPI endpoint accessible" \
        "curl -f -s http://localhost:${APP_PORT}/openapi >/dev/null"
    
    run_test "OpenAPI contains 'openapi'" \
        "curl -s http://localhost:${APP_PORT}/openapi | grep -q 'openapi'"
    
    # Lab02-specific: Test CompressionFilter
    run_test "CompressionFilter: Response compressed with gzip" \
        "curl -s -H 'Accept-Encoding: gzip' -I http://localhost:${APP_PORT}/ | grep -q 'Content-Encoding: gzip'"
    
    run_test "CompressionFilter: No warnings in logs" \
        "! podman logs ${CONTAINER_NAME} 2>&1 | grep -q 'SRVE8094W.*Cannot set header'"
    ...
    if [ "$TESTS_FAILED" -eq 0 ]; then
        ...
        echo "  👤 Client View:   http://localhost:${APP_PORT}/client?action=view&id=1"
        echo "  ➕ New Client:    http://localhost:${APP_PORT}/client?action=new"
        echo ""
        ...
        echo "  📖 OpenAPI:       http://localhost:${APP_PORT}/openapi"
        echo ""
