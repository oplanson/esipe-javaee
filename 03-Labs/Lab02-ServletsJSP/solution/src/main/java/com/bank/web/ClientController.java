 ...
 */
@WebServlet(name = "ClientController", urlPatterns = { "/clients", "/client" }, loadOnStartup = 1)
public class ClientController extends HttpServlet {
    ...
    private void updateClient(HttpServletRequest req, HttpServletResponse resp)
        ...
        Map<String, String> errors = validateUpdateInput(idParam, name, email);
        if (!errors.isEmpty()) {
            handleValidationErrors(req, resp, "edit", idParam, errors);
            return;
        }
        ...
        try {
            ...
            params.put("message", "updated");
            resp.sendRedirect(buildRedirectUrl(req.getContextPath(), "view", id, params));
    ...
    private Map<String, String> validateUpdateInput(String idParam, String name, String email) {
    ...
    private void handleValidationErrors(HttpServletRequest req, HttpServletResponse resp,
            String action, String id, Map<String, String> errors)
            throws IOException {
        ...
        for (Map.Entry<String, String> entry : errors.entrySet()) {
            params.put("error", entry.getKey() + "_" + entry.getValue());
        }

        resp.sendRedirect(buildRedirectUrl(req.getContextPath(), action,
                id != null ? Long.parseLong(id) : null, params));
    }

    private String buildRedirectUrl(String contextPath, String action, Long id, Map<String, String> params) {
        StringBuilder url = new StringBuilder(contextPath);
        url.append("/client?action=").append(action);
