...
# Docker-based testing for Lab 3 with Open Liberty and PostgreSQL
# No local Liberty installation required!

set -e

echo "=========================================="
echo "Lab 3: Docker + Open Liberty + PostgreSQL"
echo "=========================================="
echo ""
...
echo "----------------------------"
docker build -t banking-jpa-app:lab03 -f Containerfile . -q
...
if docker ps -a | grep -q banking-jpa-lab03; then
    echo "Stopping existing container..."
    docker stop banking-jpa-lab03 > /dev/null 2>&1 || true
    docker rm banking-jpa-lab03 > /dev/null 2>&1 || true
fi
...
while [ $ELAPSED -lt $TIMEOUT ]; do
    ...
    if ! docker ps | grep -q banking-jpa-lab03; then
        echo ""
        echo -e "${RED}❌ Container stopped unexpectedly${NC}"
        echo ""
        echo "Container logs:"
        echo "----------------------------"
        docker logs banking-jpa-lab03 | tail -50
        exit 1
    fi
...
if [ "$LIBERTY_STARTED" = false ]; then
    ...
    docker ps -a | grep banking-jpa-lab03
    echo ""
...
if docker exec banking-db psql -U bankuser -d bankdb -c "\dt" | grep -q "clients"; then
    ...
    echo "  docker logs banking-jpa-lab03 | grep -i flyway"
fi
...
if [ $TESTS_FAILED -gt 0 ]; then
    ...
    echo "  docker logs banking-jpa-lab03"
    echo "  docker logs banking-jpa-lab03 | grep -i error"
    echo "  docker logs banking-jpa-lab03 | grep -i flyway"
fi
...
echo "  docker logs -f banking-jpa-lab03"
echo ""
echo "View database logs:"
echo "  docker logs -f banking-db"
echo ""
echo "Stop application:"
echo "  docker stop banking-jpa-lab03"
echo ""
echo "Stop database:"
echo "  docker-compose down"
echo ""
echo "Remove application container:"
echo "  docker rm banking-jpa-lab03"
echo ""
echo "Remove application image:"
echo "  docker rmi banking-jpa-app:lab03"
echo ""
echo "Restart application:"
echo "  docker restart banking-jpa-lab03"
echo ""
...
echo ""
echo -e "${GREEN}✓ Lab 3 is running with Docker + Open Liberty + PostgreSQL!${NC}"
echo ""
