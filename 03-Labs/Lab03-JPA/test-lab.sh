...
echo "=========================================="
echo "Lab 3: JPA and Database Integration - Test"
echo "=========================================="
echo ""
...
# Check WAR file
if [ -f "target/banking-jpa-app.war" ]; then
    echo -e "${GREEN}✓ WAR file created: target/banking-jpa-app.war${NC}"
    WAR_SIZE=$(du -h target/banking-jpa-app.war | cut -f1)
    echo "  Size: $WAR_SIZE"
else
...
if jar -tf target/banking-jpa-app.war | grep -q "WEB-INF/classes/com/bank/model/Client.class"; then
    echo -e "${GREEN}✓ Client entity found${NC}"
else
    echo -e "${RED}❌ Client entity missing${NC}"
fi

if jar -tf target/banking-jpa-app.war | grep -q "WEB-INF/classes/com/bank/model/Account.class"; then
    echo -e "${GREEN}✓ Account entity found${NC}"
else
    echo -e "${RED}❌ Account entity missing${NC}"
fi

if jar -tf target/banking-jpa-app.war | grep -q "WEB-INF/classes/com/bank/service/ClientService.class"; then
    echo -e "${GREEN}✓ ClientService found${NC}"
else
    echo -e "${RED}❌ ClientService missing${NC}"
fi

if jar -tf target/banking-jpa-app.war | grep -q "WEB-INF/classes/META-INF/persistence.xml"; then
    echo -e "${GREEN}✓ persistence.xml found${NC}"
else
    echo -e "${RED}❌ persistence.xml missing${NC}"
fi

if jar -tf target/banking-jpa-app.war | grep -q "WEB-INF/classes/db/migration/V1__create_clients_table.sql"; then
