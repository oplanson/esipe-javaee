...
main() {
    ...
    # Lab03-JPA Specific Tests
    print_info "Running Lab03-JPA specific tests..."
    ...
    run_test "Database health check" \
        "curl -f -s http://localhost:${APP_PORT}/health/ready | grep -q '\"status\":\"UP\"'"
    
    # Test JPA entity operations (if API exists)
    run_test "Client API endpoint accessible" \
        "curl -f -s http://localhost:${APP_PORT}/clients > /dev/null"
    
    # Test JNDI configuration service (if exists)
    if curl -f -s "http://localhost:${APP_PORT}/validator" > /dev/null 2>&1; then
        run_test "JNDI validator page accessible" \
            "curl -f -s http://localhost:${APP_PORT}/validator > /dev/null"
        
        run_test "JNDI configuration loaded" \
            "curl -s http://localhost:${APP_PORT}/validator | grep -q 'maxTransactionAmount'"
    fi
