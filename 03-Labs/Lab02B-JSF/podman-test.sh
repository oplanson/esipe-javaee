...
# Podman-based testing for Lab 02B with Open Liberty
# No local Liberty installation required!

set -e

echo "=========================================="
echo "Lab 02B: Podman + Open Liberty Testing"
echo "JSF Client Management"
echo "=========================================="
echo ""
...
if podman images | grep -q "lab02b-jsf"; then
...
if [ $? -eq 0 ]; then
    ...
    echo "  WAR file: target/lab02b-jsf.war"
else
...
while [ $ELAPSED -lt $TIMEOUT ]; do
    ...
    if curl -s -f http://localhost:$APP_PORT/lab02b-jsf/ > /dev/null 2>&1; then
        echo ""
...
if curl -s -I http://localhost:$APP_PORT/lab02b-jsf/ | head -1 | grep -q "200\|302"; then
...
echo "  🏠 Home:          http://localhost:9080/lab02b-jsf/"
echo "  👥 Client List:   http://localhost:9080/lab02b-jsf/views/client-list.xhtml"
echo "  ➕ New Client:    http://localhost:9080/lab02b-jsf/views/client-form.xhtml"
echo ""
...
HTTP_CODE=$(curl -s -o /tmp/response.txt -w "%{http_code}" http://localhost:$APP_PORT/lab02b-jsf/)
if [ "$HTTP_CODE" = "200" ] && grep -q -i "bank\|client\|esipe" /tmp/response.txt; then
...
echo -n "Testing client list page... "
HTTP_CODE=$(curl -s -o /tmp/response.txt -w "%{http_code}" http://localhost:$APP_PORT/lab02b-jsf/views/client-list.xhtml)
...
echo -n "Testing client form page... "
HTTP_CODE=$(curl -s -o /tmp/response.txt -w "%{http_code}" http://localhost:$APP_PORT/lab02b-jsf/views/client-form.xhtml)
if [ "$HTTP_CODE" = "200" ]; then
    echo -e "${GREEN}✓ PASS${NC}"
    TESTS_PASSED=$((TESTS_PASSED + 1))
else
...
if command -v open &> /dev/null; then
    ...
    open http://localhost:$APP_PORT/lab02b-jsf/
elif command -v xdg-open &> /dev/null; then
    echo "Opening browser..."
    xdg-open http://localhost:$APP_PORT/lab02b-jsf/
fi

echo ""
echo -e "${GREEN}✓ Lab 02B is running with Podman + Open Liberty!${NC}"
echo ""
